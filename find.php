...
$result = finder("text.txt", 'aaadd'); // тут указывать файл и ключ
...
function finder($file, $key)
{
	...
	//!! 1 найти ближайший ключ-значение
    {
        ...
        $cmp = strcmp($key, $str);
        $pre_middle = $middle;  // это надо для того чтобы понять что мы ничего не нашли, если середина области не меняется, 
                                //то значит область состоит из 1 значения и больше делиться не будет.
        if ($cmp == 0) {
            // ключ найден
            while(true) {   // цикл для чтения значения
                $buffer = fread($handle, 1);
                if (ord($buffer) !== 10 && ord($buffer) !== 13 && $buffer !== false) {
                    ...
                    // так же проверяем конец файла fread возвращает false
                    $value .= $buffer;
                } else {
                    break;
                }
